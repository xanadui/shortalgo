    ...
    if position == False:
                    ...
                    while m<1:
                        try:
                            if symbol=='BTC/USDT:USDT':
                                ...
                                price = ku.fetchTicker(symbol)
                                time.sleep(30)
                                order = ku.createOrder('BTC/USDT:USDT', 'limit', 'sell', ((math.floor(income/float(price['info']['price'])*10000))/10000)*1000, float(price['info']['price'])*0.9997, {'leverage': 5})
                                ...
                                price = ku.fetchTicker(symbol)
                                time.sleep(30)
                                order = ku.createOrder(symbol, 'limit', 'sell', (math.floor((income/float(price['info']['price']))*1000)/1000)*100, float(price['info']['price'])*0.9997, {'leverage': 5})
                                ...
                                price = ku.fetchTicker(symbol)
                                time.sleep(30)
                                order = ku.createOrder(symbol, 'limit', 'sell', (math.floor(income/float(price['info']['price'])*100)/100)*10, float(price['info']['price'])*0.9997, {'leverage': 5})
                                ...
                                price = ku.fetchTicker(symbol)
                                time.sleep(30)
                                order = ku.createOrder(symbol, 'limit', 'sell', (math.floor((income/float(price['info']['price']))))/10, float(price['info']['price'])*0.9997, {'leverage': 5})
        ...
        if position == True:
            ...
            if float(price['info']['price'])/1.00875 > entryprice:
                ...
                while stopordersellvariable < 1:
                    try:
                        ...
                        time.sleep(20)
                        stopsellorder = ku.createOrder(contract, 'limit', 'buy', amount-halfsies, float(price['info']['price'])*1.0003, {'leverage': 5})
                ...
                if ku.fetchBalance()["info"]["data"]["positionMargin"] > 1:
                    ...
                    while stoptptwo <1:
                        time.sleep(10)                      
                        try:
                            stopsellordertoo = ku.createOrder(contract, 'limit', 'buy', halfsies, float(price['info']['price'])*1.0003, {'leverage': 5})
                            sellsecond+=1
                        except:
                            time.sleep(30)
                            price = ku.fetchTicker(contract)                            
            ...
            #         sellorder = ku.createOrder(contract, 'limit', 'sell', halfsies, float(price['info']['price'])*.9997, {'leverage': 5})
                ...
                if ku.fetchBalance()["info"]["data"]["positionMargin"] > 1:
                    ...
                    while nomovementstophalfsies<1:
                        try:
                            price = ku.fetchTicker(contract)
                            time.sleep(10)
                            sellorder = ku.createOrder(contract, 'limit', 'buy', halfsies, float(price['info']['price'])*1.0003, {'leverage': 5})
                ...
                while guacamole<1:
                    try:
                        sellorder = ku.createOrder(contract, 'limit', 'buy', amount-halfsies, float(price['info']['price'])*1.003, {'leverage': 5})
                        guacamole=1
                    except:
                        time.sleep(25)
                        price = ku.fetchTicker(contract)
